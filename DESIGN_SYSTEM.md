...
本次文件更新對照的程式碼範圍：commit `9f7eebe`（設計系統統一）到 `73efd97`（2026-08-05 無障礙修正與測試綠化）。
...
**深色模式規則**：語意色與 UVI 色在深色模式下**不能沿用淺色 hex**——同一色值在近黑背景上會因同時對比效應（色彩在對比背景上看起來更淡）顯得洗白。上表右側兩欄是已經調過明度與飽和度的值：相較淺色版本，深色版本必須提高明度（變亮）且提高飽和度（顏色更濃）。

做法：
- 取淺色 hex，用 DevTools 或設計工具量出 LCH 座標或 HSL 值
- 在深色背景上試色：L 軸（亮度）提高 +15～20，S 軸（飽和度）提高 +5～10
- 跟深色背景 `#0f0e0c` 做同時對比測試，確保視覺對比度足夠（不要相信單純的 WCAG AA）
- 開發時勿偷懶直接用淺色值；舊 dist/ 即因此包含被誤用的淺色值
...
**顏色不得是狀態的唯一載體**。必須另有文字、可及名稱或形狀承載相同資訊，使用者就算看不出顏色差異（色弱、色盲、強光環境、灰階顯示）也能辨識狀態。驗證方式：把畫面轉成灰階，狀態是否仍可辨識。

舉例：
- ❌ BrandHeader 只用狀態點顏色（藍／橙／紅）而無文字
- ✓ BrandHeader 狀態點搭配 contextLabel 文字（「提醒進行中」等）
- ❌ BottomNavigation 到期紅點是 aria-hidden、無可及名稱
- ✓ BottomNavigation aria-label 改為「提醒（有部位建議現在處理）」

### Tracking 狀態的特殊規則

綠色不得表示皮膚安全、產品仍有效、期限可信或不需行動。Tracking 提醒代表「使用者主動套用防護後的持續保護狀態」，不是「安全狀態」，不應有綠色或「安全」語言，也不能用勾勾圖示表示狀態。

**唯一例外**：動作按鈕（例如「記錄已補擦」）內的確認圖示屬於**操作 affordance**，不屬於狀態表示，允許沿用。
