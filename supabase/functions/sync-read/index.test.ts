...
let handleRead: typeof import("./index.ts").handleRead;

function makeRequest(method: string): Request {
  return new Request("https://api.test/sync-read", {
    method,
...
beforeEach(async () => {
  ...
  ({ handleRead } = await import("./index.ts"));
});
...
describe("sync read boundary", () => {
  it("approved origin 的 OPTIONS 回 204，且不要求 JWT", async () => {
    const response = await handleRead(makeRequest("OPTIONS"));
  ...
  it("拒絕非 POST method，且不進入驗證", async () => {
    const response = await handleRead(makeRequest("GET"));
  ...
  it("未登入的 POST 仍交給 requirePermanentUser 回 401", async () => {
    const response = await handleRead(makeRequest("POST"));
