...
import { hashClientFingerprint, SlidingWindowRateLimiter } from "../_shared/rate-limit.ts";
import {
  corsHeaders,
  errorResponse,
  jsonResponse,
...
const limiter = new SlidingWindowRateLimiter({ maxRequests: 5, windowMs: 10 * 60 * 1000 });
...
export async function handleFeedback(request: Request): Promise<Response> {
  ...
  try {
    ...
    return toResponse(errorResponse({
      ...
      message: error instanceof FeedbackValidationError ? error.message : "JSON 格式不正確"
    }));
  }
  ...
  if (supabaseUrl === undefined || serviceRoleKey === undefined) {
    return toResponse(errorResponse({ status: 500, code: "SERVER_ERROR", message: "回報服務尚未完成設定" }));
  }
  const client = createClient(supabaseUrl, serviceRoleKey, {
    auth: { persistSession: false, autoRefreshToken: false }
  });
  const dedupeHash = await hashClientFingerprint([fingerprint, canonicalFeedback(parsed)]);
  ...
  if (existing.error !== null) {
    return toResponse(errorResponse({ status: 500, code: "SERVER_ERROR", message: "目前無法處理回報" }));
  }
  ...
  const insertResult = await client.from("feedback_submissions").insert({
  ...
  if (insertResult.error !== null) {
    ...
    return toResponse(errorResponse({ status: 500, code: "SERVER_ERROR", message: "回報尚未送出" }));
  }
