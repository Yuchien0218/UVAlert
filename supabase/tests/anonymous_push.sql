...
select ok(
  not has_function_privilege('anon', 'public.settle_push_schedule(uuid,uuid,text,timestamptz,text,timestamptz)', 'execute'),
  'anon cannot execute settlement'
);
select ok(
  not has_function_privilege('authenticated', 'public.settle_push_schedule(uuid,uuid,text,timestamptz,text,timestamptz)', 'execute'),
...
select is(
  (
    select count(*)
    from public.settle_push_schedule(
