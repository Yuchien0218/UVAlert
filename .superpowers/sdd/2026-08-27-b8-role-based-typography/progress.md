...
Preview blank fix: fix round 2 red reproduced (2026-08-29 — omitted `baseUrl` plus blank `VITE_API_BASE_URL` and nonblank Supabase env still issued request to raw whitespace URL instead of `/v1`; public fetch seam caught constructor env reread)
Preview blank fix: fix round 2 green complete (constructor now consumes factory-resolved `baseUrl` or `/v1` fallback only; explicit blank caller `baseUrl` still disables sync; cloud focused 1 file / 11 tests and adapter/helper focused 3 files / 21 tests pass)
Preview blank fix: fix round 2 full verification fresh (full `pnpm check` exit 0 after workspace typechecks, 91 files / 662 tests, ESLint, and Stylelint; `pnpm build` exit 0 with existing chunk-size and unset `VITE_PUBLIC_SITE_URL` warnings)
