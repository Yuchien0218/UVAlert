...
describe("planReapplication", () => {
  it("建立一個群組與不同 snapshot 的互斥 events，只更新選中部位", () => {
    const initial = planStartSession(
      makeStartSessionCommand({
        idPrefix: "reapply",
        zones: [
          {
          ...
          {
            zoneInstanceId: "zone-c",
            trackingEventId: "track-c",
            methodEventId: "method-c",
            bodyZoneCode: "ears",
            customLabel: null,
            skinExposureStatus: "exposed",
            methodCertainty: "confirmed",
    ...
    const previousC = initial.projection.zones.find(
      (zone) => zone.zoneInstanceId === "zone-c"
    )!;
    const snapshotA = makeProductSnapshot({
        ...
        applications: [
          {
            ...
            productLabelSnapshot: snapshotA
          },
          {
            eventId: "app-b",
    ...
    expect(
      plan.projection.zones.find((zone) => zone.zoneInstanceId === "zone-c")
    ).toEqual(previousC);
  });
});
