...
import { ReapplyCommandV1Schema, fingerprintProductLabelSnapshot } from "@sunshield/contracts";
...
const clock = { status: "trusted", trustedNow: "2026-08-01T10:30:00.000Z", connectivity: "online" } as const;
...
describe("LocalSessionRepository.reapply", () => {
  it("原子提交 confirmation group、events、projection、revision、sequence 與 receipt，重送冪等", async () => {
    ...
    const repository = new LocalSessionRepository({ database, sourceContextId: "test" });
    ...
    expect(await database.ApplicationConfirmationGroups.get("reapply-group")).toBeDefined();
    expect(await database.ApplicationEvents.get("reapply-event")).toBeDefined();
    expect((await database.ProtectionSessions.get(start.sessionId))?.revision).toBe(2);
    expect((await database.ClientSequences.get(["device-1", start.sessionId]))?.lastSequence).toBe(2);
    expect(await database.CommandReceipts.get("reapply-idem")).toBeDefined();
  });

  it("拒絕 stale product snapshot 且不留下部分資料", async () => {
    const database = new SunshieldDatabase(`reapply-stale-${crypto.randomUUID()}`);
    databases.push(database);
    const repository = new LocalSessionRepository({ database, sourceContextId: "test" });
    const catalog = new LocalProductCatalogRepository(database);
    ...
    const command = ReapplyCommandV1Schema.parse({
      ...
      clientSequence: 2, clientCreatedAt: clock.trustedNow, expectedRevision: 1,
      payload: { applicationConfirmationId: "stale-group", appliedAt: "2026-08-01T10:25:00.000Z", applications: [{
        eventId: "stale-event", zoneInstanceIds: ["stale-zone-face"], sourceProductId: product.productId,
        productSnapshotFingerprint: "stale-fingerprint", productLabelSnapshot: product.currentSnapshot
      }] }
    });
    ...
    expect(await database.ApplicationConfirmationGroups.get("stale-group")).toBeUndefined();
    expect((await database.ProtectionSessions.get(start.sessionId))?.revision).toBe(1);
  });

  it("transaction 中途失敗時回滾 group、event 與 revision", async () => {
    const database = new SunshieldDatabase(`reapply-rollback-${crypto.randomUUID()}`);
    databases.push(database);
    const repository = new LocalSessionRepository({ database, sourceContextId: "test" });
    ...
    const command = ReapplyCommandV1Schema.parse({
      commandVersion: "1.0.0", commandType: "record_reapplication", commandId: "rollback-command", idempotencyKey: "rollback-idem",
      owner: { type: "guest", localVisitorId: "visitor-1" }, deviceLocalId: "device-1", sessionId: start.sessionId,
      clientSequence: 2, clientCreatedAt: clock.trustedNow, expectedRevision: 1,
      payload: { applicationConfirmationId: "rollback-group", appliedAt: "2026-08-01T10:25:00.000Z", applications: [{ eventId: "rollback-event", zoneInstanceIds: ["rollback-zone-face"], sourceProductId: null, productSnapshotFingerprint: fingerprintProductLabelSnapshot(snapshot), productLabelSnapshot: snapshot }] }
    });
    vi.spyOn(database.ProtectionZoneStates, "bulkPut").mockRejectedValueOnce(new Error("injected failure"));

    expect(await repository.reapply(command, clock)).toMatchObject({ ok: false, code: "PERSISTENCE_ERROR" });
    expect(await database.ApplicationConfirmationGroups.get("rollback-group")).toBeUndefined();
    expect(await database.ApplicationEvents.get("rollback-event")).toBeUndefined();
    expect((await database.ProtectionSessions.get(start.sessionId))?.revision).toBe(1);
