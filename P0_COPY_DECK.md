...
| --- | --- |
| title | 本機資料 |
| body | P0 的產品、提醒、事件與草稿保存在目前裝置。清除網站資料、解除安裝或更換裝置可能使資料消失，可以先匯出一份檔案自己保管。 |
| review_status | LEGAL_REVIEW |

### CP-DATA-EXPORT-001：匯出本機資料（2026-08-07 新增）

| 欄位 | 內容 |
| --- | --- |
| title | 匯出本機資料 |
| body | 把目前裝置上的產品、提醒、事件與偏好存成一個檔案，由你自己保管。檔案不會上傳，也不會傳給我們。 |
| primary_cta | 匯出檔案 |
| review_status | LEGAL_REVIEW |

**不得**寫成「備份到雲端」、「同步」或任何暗示跨裝置恢復的說法——
P0 只有匯出，沒有匯入。

### CP-DATA-EXPORT-002：匯出成功

| 欄位 | 內容 |
| --- | --- |
| title | 已匯出 |
| body | 檔案已存到你的裝置。請自行保管；目前還不能把這個檔案匯回 App。 |
| review_status | LEGAL_REVIEW |

### CP-DATA-EXPORT-003：匯出失敗

| 欄位 | 內容 |
| --- | --- |
| title | 匯出沒有完成 |
| body | 檔案沒有成功產生，你的資料沒有任何改變。可以再試一次。 |
| primary_cta | 再試一次 |
| review_status | LEGAL_REVIEW |
...
| --- | --- |
| title | 清除全部本機資料？ |
| body | 這會清除目前裝置上的產品、提醒、事件、草稿、最後氣象快照與設定。這項操作無法在本機復原。想留一份紀錄的話，可以先匯出檔案。 |
| primary_cta | 清除全部資料 |
| secondary_cta | 先匯出 |
| tertiary_link | 取消 |
| review_status | LEGAL_REVIEW |

清除前必須提示可先匯出（2026-08-07 裁決）。次要按鈕改為 `先匯出`，
`取消` 降為文字連結——破壞性操作旁邊放的應該是挽救手段，不是單純的退出。
