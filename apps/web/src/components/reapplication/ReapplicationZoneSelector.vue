...
defineProps<{ zones: ZoneProjection[]; selectedZoneIds: string[]; suggestedZoneIds: string[]; error: string | undefined }>();
const emit = defineEmits<{ suggested: []; all: []; toggle: [zoneId: string] }>();
</script>
...
<template>
  <fieldset class="app-card reapply-section" :aria-describedby="error ? 'zone-selection-error' : undefined">
    ...
    <p v-if="error" id="zone-selection-error" class="form-error" role="alert">{{ error }}</p>
  </fieldset>
</template>
...
.zone-choice small { color: var(--color-tracking); }
