...
const warning = computed(() => {
  ...
  const copy: Record<string, { title: string; body: string }> = {
     ...
     *
     * 它們原本寫「不會產生 120、40 或 80 分鐘期限」「暫時無法建立補擦倒
     * 數」——規則改動之後這兩句是假的：不確認標示現在會用 120 分鐘保守
     * 預設建立倒數。留著等於警告一件不會發生的事。
     *
     * 這是**移除已不成立的敘述**，不是改寫文案——上面那段註解說的「不要
     * 在這裡改寫 Copy Deck 條目」仍然有效。若之後想改成「未確認標示，使
     * 用 120 分鐘保守預設」之類的提示，那是新文案，要先裁決。
     *
     * 保留的三則都仍然為真：過期／異常／不適確實不建立倒數。
     */
  };
