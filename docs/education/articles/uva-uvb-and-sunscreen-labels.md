...
UVB 比較常和皮膚曬紅、曬傷有關；UVA 能深入皮膚，與膚色變深及長期光老化有關。防曬乳上的 SPF 主要是 UVB 防護指標，PA 則是 UVA 防護標示。選防曬乳時，不要只看其中一個數字，也不要把 SPF 30 理解成可以在太陽下待 30 倍時間。

## UVA、UVB、UVC 怎麼分？

- **UVA**：波長較長，能深入皮膚較深層，與膚色變深、光老化等長期影響相關。
- **UVB**：較容易造成皮膚表層的紅、熱、痛與曬傷。
- **UVC**：自然陽光中的大部分會被大氣層阻隔；日常防曬主要關注 UVA 與 UVB。
...
數字或加號越高，不等於防護可以無限延長，也不代表可以少擦、少補擦。產品測試是在特定條件與用量下進行；如果塗得太少、被汗水沖掉或用毛巾擦掉，實際防護會受到影響。
...
FDA 特別提醒，SPF 不是「原本能曬 1 小時，就能用 SPF 30 曬 30 小時」的時間換算。太陽強度、地點、時段、用量與產品是否被擦掉，都會改變實際曝曬量。防曬晴報員 的 120 分鐘提醒是保守的補擦提醒，不是 SPF 的計算結果。
