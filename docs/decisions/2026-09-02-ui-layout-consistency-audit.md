...
| 稽核項 | 前置條件／調整 |
| --- | --- |
| ~~**§1 導覽出口**~~ | **已完成（2026-09-02）**——見下方「第二輪處置」。 |
| ~~**§3 / P2 原生 Radio → 膠囊組**~~ | **已完成（2026-09-02）**——見下方「第二輪處置」。 |
| ~~**§4 / P3 設定頁圖示錨點**~~ | **已完成（2026-09-02）**——見下方「第二輪處置」。 |
| ~~**§2 深色表面語意**~~ | **已完成（2026-09-03）**——見下方「第三輪處置」。（原稿寫「`--surface-inverse` 引用 0 次」，那在 2026-09-01 分享卡落地後已不成立。） |
| ~~**§7.4 ProductEligibilityNotice 圖示基線**~~ | **已量測，不改（2026-09-02）**——見下方「第二輪處置」。 |

**回寫落點**：`docs/decisions/README.md` 的「裁決 → 回寫落點」表已補一列指向本節。

## 第二輪處置（2026-09-02，使用者逐項裁決後）

### §1 導覽出口 → 已完成，但採用的不是原提案

實際盤點後，「流程用叉叉／下鑽用箭頭」這條規則**已經被遵守九成**，唯一站錯邊的是 `NotificationSettingsPage`。所以這不是空白的設計題，是一頁用錯族群——只改那一頁。

原提案的「一律**左上**箭頭＋刪頁尾連結」不採用：左上／右上仍在 `2026-08-30-pending-decisions §2／§12.2` 待裁決，本次只換圖示不動位置；刪頁尾連結與 `2026-08-27` 抽 `BackToMoreLink` 的結論衝突。

規則與守門見 `2026-09-02-secondary-page-exit-rule.md`。

### §3 原生 Radio → 已完成

`NotificationSettingsPage` 的「再次提醒頻率」改用共用的 `.choice-grid`。**原生 radio 保留**（`accent-color` 上色，不是藏起來自刻圓點），所以鍵盤操作、方向鍵在群組內移動與 `radiogroup` 語意全部不變。選取色沿用 `.choice-grid label:has(input:checked)`，沒有自刻。

### §4 設定頁圖示錨點 → 已完成

`DataSettingsPage` 三個區塊標題各配一個 32px 圖示。**沿用 `MorePage` 卡片列的既有排法**，不發明第四種「圖示＋標題」樣式；`IconLead` 的 40／56 是給頁面標題與空狀態的，三張設定卡各放一個會太重。

第一個用 `more-data`——那正是「更多」頁上通往這裡的那張卡的圖示，點進來之後圖示還在原地。

匯出卡的按鈕原本也有 `tool-download`，標題接手之後拿掉按鈕那顆：同一張卡出現兩個一樣的圖示是噪音，而按鈕有完整文字標籤。

「跨裝置同步」區塊**刻意不配圖示**：它依 `2026-08-29-settings-data-sync-merge.md` 是無卡片外框的次要區塊，加圖示會把它拉回第一層。

### §7.4 圖示基線 → 已量測，不改

稿子說「圖示中心比標題文字重心略高出約 2~3px」。實機量測（375×812、`getClientRects()` 取標題第一行的實際字框）：
...
偏移是 0.75px，不是 2~3px，而且方向是圖示略高——**低於任何可感知的門檻**。稿子建議的 `margin-top: 1px` 會把它推過頭 0.25px，同時塞進一個魔術數字。

所以這一項不改。截圖目視也確認圖文對齊良好。**先量再改的價值就在這裡**：這是一個看起來很合理、但前提量出來不成立的建議。

## 第三輪處置（2026-09-03）

### §2 深色表面語意 → 已完成，並且加了守門

`DESIGN.md` 第二節新增〈深色表面的使用範圍〉。判準寫成**「這一塊是不是要被單獨看的主角」**，不是「這一塊重不重要」——後者每個人都會覺得自己那塊符合。
...
第七節（高度與深度）那一列原本寫「保留給未來需要深色資料表面的場合」，指向新的範圍規則；第十節那一列同步。

**原稿有一處已過期**：「`--surface-inverse` 引用 0 次」在 2026-09-01 分享卡落地之後就不成立了，它現在是唯一的消費者。

**沒有裁決 2026-08-23 那三個規劃落點**（設定流程步驟外框、產品標示編輯、五日預報強調欄）。依新規則，其中前兩個是表單容器、已經不成立；但那三處是另一條線的待辦，這條規則只說明它們落在哪一邊，要保留任何一個需要另外裁決並註明理由。

### 不只寫進文件：加了白名單守門

`apps/web/src/assets/darkSurfaceScope.test.ts` 掃全 `src`，任何未登記的 `var(--surface-inverse)`／`var(--color-surface-dark*)`（以及 TypeScript 裡的字串取值，分享圖走 canvas）都會紅。白名單每一列要寫明**為什麼它是主角**。

四個方向分開守：未登記的用法會紅、白名單寫了卻沒人用會紅、`DESIGN.md` 少了那一節會紅、規則只寫「可以用」漏掉「不可以用」也會紅。全部破壞驗證過。
