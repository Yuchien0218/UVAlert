...
| loading 文案   | `SunLoader` 有預設「載入中」，頁面另有「正在讀取裝備清單／目前提醒狀態／本機資料概況」等具體名稱 | 保留就近、具情境的 label；只統一呈現元件，不把不同 accessible name 壓成模糊的「載入中」。                                                             |
...
- 非同步狀態必須說明正在讀取的資料，並作為 `SunLoader` 的 accessible name。
...
- 以 TDD 新增頁面測試；先證實找不到 `SunLoader` 而失敗，再改為 `SunLoader label="正在讀取本機資料概況"` 後通過。
