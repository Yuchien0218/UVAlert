...
~~**`ContextSelector` 不做高度動畫**~~ **（2026-09-04 已補做，見文末）**（只換 chevron）。它的展開區是**跨群組共用、內容會換的**同一個面板，包進 `DisclosurePanel` 會讓 `activeGroup` 為 undefined 時直接炸掉，而且切換群組也會跟著跑高度動畫。這是元件結構問題，不是動效問題，留給之後單獨處理。
...
## 九、`ContextSelector` 的展開區（2026-09-04 補做）

批次 4 當時把它跳過，理由是「元件結構問題，不是動效問題」。那個判斷是對的，
補做時的正解也正是**改結構**而不是寫動畫。

**實測的高度落差**（390px）：說明 24px、室內子選項 171px、水上子選項 192px、
空 0px。從說明切到室內是 **+147px 的瞬跳**，值得處理。

**為什麼共用區接不上 `DisclosurePanel`**：那個元件做的是「0 ↔ 內容高度」，
而共用區是「內容高度 ↔ 另一個內容高度」；而且 `activeGroup` 收合時會變
`undefined`，模板直接炸。

**做法**：一個群組一個面板（室內、水上、說明共三個），每個都回到單純的開／關。

一併處理的兩個細節：

1. **說明的內容不再隨展開狀態變 null。** 原本 `selectedDescription` 在
   `openGroup !== null` 時回傳 null——收合的當下內容就沒了，高度動畫等於在
   空盒子上跑。改成內容一直在，由 `descriptionOpen` 決定開不開。
2. **順帶修好一個無障礙問題**：`aria-controls` 指的 id 以前只有展開時才在
   DOM 裡，收合時指向一個不存在的元素（`DESIGN.md` 第五節要求觸發器與被控
   制區塊齊備）。拆開之後 id 永遠存在，收合時由 `DisclosurePanel` 標成 `inert`。

**規則五**：切換群組時一個面板收合、另一個展開——那是同一個動作的兩半，
不是規則五要擋的「兩個各自獨立的元素同時動」。

守門五條（`contextSelectorDisclosure.test.ts`），兩種破壞各自命中不同條。
實機四種狀態都量過：任何時刻恰好一個面板打開，總高度等於那一個面板的高度。
