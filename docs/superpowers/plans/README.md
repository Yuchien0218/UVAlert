...
| [`2026-08-26-codebase-consolidation-audit.md`](2026-08-26-codebase-consolidation-audit.md) | **盤點清單，7 個 Open Question 已於 2026-08-26 裁決（見文件內表格）。已完成：C2（刪 Claude Design 匯出，commit `3f38a9d`）、C1（drift 測試 `packages/ui/src/tokens.test.ts`，commit `b1cf1db`）、A1＋A6（Stylelint ＋ CLAUDE.md 硬規則，已併進 `pnpm check`）。** 其餘 A2–A5／B／D–G 未動工、彼此獨立可分 session 挑做。C1 抓到 5 類 token 落差待 D2 裁決。現況數字動工前需重新 grep 核對 | [`../../decisions/2026-08-25-*.md`](../../decisions/) 六份、[`../../../DESIGN.md`](../../../DESIGN.md) §5／§10／§12／§13、`packages/ui/src/styles.css`、`apps/web/src/assets/app.css`、`stylelint.config.mjs` |
