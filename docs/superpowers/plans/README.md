...
| [`2026-08-26-codebase-consolidation-audit.md`](2026-08-26-codebase-consolidation-audit.md) | **盤點清單，7 個 Open Question 已於 2026-08-26 裁決（見文件內表格）。已完成：C2（刪除兩個 Claude Design 匯出資料夾，commit `3f38a9d`）、C1（`DESIGN.md`↔`styles.css` drift 測試 `packages/ui/src/tokens.test.ts`）。** 其餘 Task A／B／D–G 未動工、彼此獨立可分 session 挑做；Task A（lint／CI）是治本，建議接著做。C1 抓到 5 類 token 落差待 D2 裁決。現況數字動工前需重新 grep 核對 | [`../../decisions/2026-08-25-*.md`](../../decisions/) 六份、[`../../../DESIGN.md`](../../../DESIGN.md) §5／§10／§12／§13、`packages/ui/src/styles.css`、`apps/web/src/assets/app.css` |
