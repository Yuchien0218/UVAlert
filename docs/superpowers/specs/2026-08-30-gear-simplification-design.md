# 裝備區簡化：規格（三件裁決已定案，未動工）

**狀態：第五節的三個問題已於 2026-08-30 裁決；程式碼尚未動工。** 這份文件回答 `docs/decisions/2026-08-30-pending-decisions.md` 第四節列的三個問題，並補上探索後才發現的第四個問題——那個問題會改變這件事的性質。
...
## 五、三件裁決（2026-08-30 已定案）

| 問題                 | 裁決                     | 影響                                                                                                                                   |
| -------------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| 耐水期限要不要保留？ | **保留，收合成選填**     | 包裝標示問答**不刪除**，只是預設收起。耐水期限（40／80 分鐘）能力完整保留，倒數也保有「比 120 分鐘更嚴格」的可能。§2 問題 1 的建議成立 |
| 「好不好用」的形式？ | **三檔：好／普通／不好** | `usageRating: z.enum(["good","ok","bad"]).nullable()`                                                                                  |
| 價格要記幣別嗎？     | **不記幣別**             | `priceTwd` 只存新台幣整數，不做匯率與多幣別顯示                                                                                        |

**因此 §2 問題 2 的答案定案為「不需要資料遷移」**——包裝標示欄位全部留著，沒有任何欄位被移除，`superRefine` 那個例外不會踩到。剩下的 schema 改動只有新增兩個選填欄位。
