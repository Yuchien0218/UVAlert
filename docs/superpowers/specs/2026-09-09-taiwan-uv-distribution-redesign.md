...
**狀態**：產品端已確認；2026-09-09 補充未選地區仍顯示全臺資料
...
- 未設定或明確略過地區時，仍要獨立呼叫全臺預報 API；只要全臺資料取得成功，就顯示地圖、圖例與 22 縣市清單。地圖不顯示所在地標記，地區列顯示「尚未設定地區」與「設定地區」，不捏造地區名稱。
...
11. 未選擇或明確略過地區時，不呼叫地區專屬五日預報 API，但仍呼叫全臺預報 API；取得資料後仍渲染地圖、圖例與清單，且沒有所在地標記。
