...
- [ ] 設定流程**不出現任何防護方式選項**；只提供追蹤部位核取清單。（2026-08-07 裁決）
- [ ] 追蹤中的部位一律送出 `skinExposureStatus=exposed`、`methodComponents=[sunscreen]`。
- [ ] 遮蔭只記在 Session 情境，不得寫入部位方法。
- [ ] 無法確認是否已擦防曬時允許返回／離開，不建立 Session。
- [ ] 建立 Session 必然需要產品標示與塗抹時間；首次使用者須先完成產品標示。
- [ ] 全部衣物覆蓋時不要求或偽造塗抹時間。
