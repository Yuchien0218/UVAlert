...
| **深色表面體系** | `{colors.surface-dark}` 用於設定流程步驟外框、產品標示編輯、五日預報強調欄（第三節第 3 點與第二節色彩表）；`surface-dark-elevated`／`surface-dark-soft`／`on-dark-soft` 為其配套 | 三個落點都還是淺色。`--surface-inverse` 被引用 0 次，`--color-surface-dark` 只出現在 `--surface-inverse` 的定義裡 | **規範有效，尚未套用。** 2026-08-23 深色面板從「首頁倒數」改定位到這三處（見第七節），但改定位之後沒有實作。**這一列的用途是防止誤判**——2026-08-30 的未使用宣告稽核一度把這 4 個 token 判成「因設計變更而失效、可收掉」，差點刪掉還有效的規範。套用範圍是三個元件的視覺重做加深色底對比度重驗，要另外排程，不併進 token 收斂。詳見 `docs/decisions/2026-08-30-unused-declaration-audit.md` |
