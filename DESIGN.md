...
layout:
  app-shell-max: 480px
  reading-shell-max: 672px
  page-content-max: 544px
  content-max: 752px
  control-max: 416px
  tap-target: 44px
...
3. **濃縮咖啡深色表面**（`{colors.surface-dark}`）— 只用於「要被單獨看的主角」，見下方〈深色表面的使用範圍〉
...
| `{colors.surface-soft}`          | `#F7EDE1` | `hsl(33, 58%, 93%)` | 區塊分隔、來源區塊（2026-08-30 拿掉「衛教引言底」——那與 `{colors.accent-blush}` 的指派衝突，已裁決引言底維持 `{colors.surface-cream-strong}`）            |
...
深色表面是這套配色裡**最強的一種強調**——象牙底上突然出現一塊濃縮咖啡，讀起來是「這一塊跟其他東西不同」。強調用在每一塊上就等於沒有強調，所以它需要一條範圍規則，而不只是一組色票。

**判準是「這一塊是不是要被單獨看的主角」**，不是「這一塊重不重要」。

|              |                                                                                      |
| ------------ | ------------------------------------------------------------------------------------ |
| **可以用**   | 要被當成儀器讀數看的即時核心看板；分享圖裡的主角卡（唯一被截圖傳出去的那一塊）       |
| **不可以用** | 一般設定頁、表單、清單、內容容器——這些是「一連串同級的東西」，其中任何一塊都不是主角 |

目前唯一的落點是**裝備分享卡**（`GearShareCard.vue`，2026-09-01），走 `--surface-inverse`。

**2026-08-23 規劃的三個落點（設定流程步驟外框、產品標示編輯、五日預報強調欄）仍未實作，而且其中前兩個依這條規則已經不成立**——它們是表單容器，不是主角。這條規則**不裁決**那三處要不要做，只是說明按目前的判準它們落在哪一邊；真要保留其中任何一個，要另外裁決並在此註明理由。相關背景見第十節的深色表面體系那一列。

深色底上的文字與圖示**一律重驗對比度**：琥珀金 `{colors.accent-amber}` 在 `{colors.surface-dark}` 上只有 4.49，過得了大字級但過不了一般內文的 AA（見 `gearShareCard.test.ts` 的實測紀錄）。
...
| 等級        | Token                    | Hex       | HSL                  |
| ----------- | ------------------------ | --------- | -------------------- |
| 低量級 0–2  | `{colors.uvi-low}`       | `#4B739E` | `hsl(211, 36%, 46%)` |
| 中量級 3–5  | `{colors.uvi-moderate}`  | `#946800` | `hsl(42, 100%, 29%)` |
| 高量級 6–7  | `{colors.uvi-high}`      | `#B25721` | `hsl(22, 69%, 41%)`  |
...
> **2026-09-02：分享卡的頁尾是「不可隱藏」清單的已裁決例外。** 分享圖的頁尾原本是這段安全註記，使用者裁決改成日期。代價（分享圖的收圖者沒有 App 脈絡，是這條規則最需要它的場景）與日後要補回來的最小做法，完整記在 `docs/decisions/2026-09-02-share-card-footer-date.md`。**這是唯一的例外，App 內各頁不受影響。**
>
...
> **2026-08-30：`<strong>` 與這條規則的 token 缺口。** 「標籤與強調短語 500」先前只寫在這句散文裡，沒有對應的 token，也沒有全域規則——於是 `<strong>` 落到瀏覽器預設的 **700**（實測 `/settings/data` 8 個全中；全站 19 個用到 `<strong>` 的檔案有 11 個完全沒覆寫字重）。已在 `apps/web/src/assets/app.css` 補全域 `strong { font-weight: 500 }`。
>
> **`--font-weight-*` 目前有 7 個角色**（page-title／section-title／card-title／body／supporting／caption／nav-label）**，沒有「強調短語」這一個**，所以那條規則只能寫字面值。借用 `--font-weight-caption` 會誤導——caption 指的是短註腳、時間戳、eyebrow、badge，不是行內強調。**要補這個 token 的話，frontmatter 與 `styles.css` 要一起加**，`tokens.test.ts` 才守得到。
...
**2026-09-03 起，這一頁的入口卡只有標題，沒有任何說明文字**（使用者裁決）。

這推翻了 B9 第一輪的逐項分類結果。當時的規則是「純重述標題的刪除、講涵蓋範圍的縮短、決策條件與健康／安全邊界一律常駐」，七張裡四張因此留著說明——結果是**六列有說明、一列沒有**，那一列比其他矮一截，圖示的垂直位置也和別人對不上（2026-09-03 稽核 §E）。稽核提的解法是「補一句」，使用者選了相反的方向。

**代價要寫清楚**：原本那四句是「進入前就先說清楚」的邊界（醫療邊界、免登入與同步、不裝也能用、免登入回報）。拿掉之後，這些話**改由目的頁自己承擔**——四頁都已經在頁首說了同一件事，所以資訊沒有消失，只是使用者要點進去才看得到。`MorePage.test.ts` 守著這一點：四個目的頁少了那句話會紅。

沒有說明的卡片不渲染空的 `<small>`，不要用空元素撐間距。
...
**`text-link`** — 內文連結，`{colors.primary-text}`（**不是** `{colors.primary}`——填色用的那顆當文字時在卡片上只有 4.37:1，理由見第十二節）。按下時加底線。

#### 連結的尾綴符號

**2026-09-03 訂定。** 2026-09-03 的稽核把「純底線／`→`／`›` 三種並存」列為疑似不一致，實際盤點後**現況已經是對的**——三者是三種角色，而且 `›` 早在 2026-08-31 就收斂成 `ChevronLink` 元件（5 個使用點）。這裡只是把口說的規則寫下來，免得下一次盤點又把它當成漂移：

| 尾綴 | 意思                               | 實作                                                    |
| ---- | ---------------------------------- | ------------------------------------------------------- |
| `›`  | **往下鑽一層**，那邊還有東西可看   | `ChevronLink`（五日預報、最近事件、部位狀態、雲端資料） |
| `→`  | **改用另一種做法**，留在同一件事裡 | `/setup` 的「改為填寫完整的防曬乳包裝標示」             |
| 無   | 其餘一切                           | `.text-link`                                            |

新增連結時先問「使用者按下去之後，是往更深一層，還是換一條路做同一件事」。兩者都不是就不要加符號。
...
**2026-09-03 訂定。** 在此之前沒有任何規則說「這顆按鈕的字該置中還是靠左」，
於是同一個畫面上會同時出現兩種——`/setup` 的「只選建議部位」置中、正上方的
「1 分鐘前」靠左，兩顆長得一樣大。
...
| 形狀                                             | 對齊                            | 例子                                                        |
| ------------------------------------------------ | ------------------------------- | ----------------------------------------------------------- |
| **獨立按鈕**：內容只有一個短標籤（或標籤＋圖示） | **置中**                        | `.button` 全族、`.time-option`、情境磚                      |
| **列**：左邊標籤、右邊值或 chevron，佔滿一整行   | **靠左**（`text-align: start`） | `ChevronLink`、裝備清單項、`.label-question__row`、最近事件 |

分辨方式：**問「這一行的右邊有沒有東西」**。有值、有 chevron、有時間戳的是
列，靠左；右邊什麼都沒有、文字自己在一塊區域裡的是按鈕，置中。

**flex 容器要用 `justify-content`**。按鈕內層若是 `display: flex`（例如
`.time-option__label` 要讓數字與單位對齊基線），`text-align` 對它無效——
只改 `text-align` 會看起來完全沒反應。
...
| `--page-stack-gap`         | 20px | **預設**（2026-08-30 從 24px 收緊一級，使用者回饋間距太鬆） | 18 頁 |
...
**目前沒有例外。** 首頁的 20px 原本是唯一的具名例外（當時預設是 24px），同日預設收緊到 20px 之後兩者同值，首頁已改回 `--page-stack-gap`。

### 卡片內距（card-padding）

**2026-08-30 收斂。** 上面第四節從很早就訂了「卡片內距預設 20px」，但那條規則在程式碼裡是 **28 處各自寫 `padding: var(--space-5)`**——規則寫在文件裡、值散在 28 個檔案裡，想整批調鬆緊就得逐一改。這跟 page-stack 收斂前是同一種問題。

| Token            |   值 | 用在                     |  目前 |
| ---------------- | ---: | ------------------------ | ----: |
| `--card-padding` | 20px | **所有卡片／面板的內距** | 28 處 |
...
| 深色表面 | `{colors.surface-dark}` 底、無陰影 | 只用於「要被單獨看的主角」——即時核心看板與分享圖主角卡；一般設定、表單、清單禁用。範圍規則見第二節〈深色表面的使用範圍〉（2026-08-23 起首頁倒數不再使用） |
...
從 **16 / 20 / 24 / 32 / 40 / 56** 六個檔位選，不發明新數值。

| 檔位 | 角色                                 | 怎麼用                        |
| ---: | ------------------------------------ | ----------------------------- |
|   16 | 文字行內的輔助圖示（按鈕內、標籤旁） | `<Icon :size="16" />`         |
|   20 | 清單列、次要位置                     | `<Icon :size="20" />`         |
|   24 | 下排導覽、按鈕、區塊標題             | `<Icon :size="24" />`（預設） |
|   32 | 卡片或功能入口的主要視覺             | `<Icon :size="32" />`         |
|   40 | **與標題並排的領銜圖示**             | `<IconLead>`                  |
|   56 | **空狀態裡唯一的視覺主體**           | `<IconLead size="hero">`      |

> **2026-08-31 新增 40 與 56（量表上緣）。** 使用者回報「畫了很多圖示卻都沒感覺、都太小了」。清點後成因很明確：36 個使用點裡 **20 個是 20px**，而整個 App 沒有任何圖示大於 32px——缺的是量表的上緣。
>
> 40 與 32 的差別不是「稍微大一點」，是角色不同：**32 是卡片裡的一個元素，40 與標題平起平坐**。56 再進一步，是空狀態裡沒有其他內容時唯一的視覺主體。
>
> **40 與 56 一律走 `apps/web/src/components/common/IconLead.vue`**，不在各處各寫一次 `:size="40"`。使用者的原話是「怕大小改一個又跑掉」——這兩個檔位出現在衛教分類卡、衛教主題頁標題、提醒頁空狀態這三個離很遠的地方，散成字面量必然漂移。要調領銜圖示的大小，改那一個檔案。
>
> **2026-09-05：56 檔位終於接上空狀態了。** 這個檔位的定義一直是「空狀態裡唯一的視覺主體」，但全站真正的空狀態元件 `EmptyStateCard` 原本**完全沒有圖示**，`size="hero"` 只用在 `HomeNightNotice` 一個地方——**檔位說它是給空狀態的，空狀態卻沒有用它。**
>
> `EmptyStateCard` 現在有一個**選填**的 `icon`。判準：**只有「本來就沒有東西」的空狀態配圖示，`role="alert"` 的讀取失敗不配**——56 是為「旁邊沒有別的內容」設計的，而讀取失敗是暫時性的狀況，不是一個章節；配一顆 56px 的圖示會把「等一下再試」放大成「這裡就是這樣」。
>
> 圖示與各自的**入口用同一顆**（`/products` 用底部導覽的 `nav-gear`、`/help` 用 MorePage 那一列的 `more-about`）——收起來的東西與它的入口用同一個記號。
>
> **`/forecast` 的空狀態刻意不配**：`FiveDayUvCard` 頂端已經有一顆 24px 的 `feature-uv-forecast`，再加 hero 會變成同一張卡兩顆圖示，正是這個檔位定義排除的情況。

> **56 不是萬用的放大。** 那是為「旁邊沒有別的內容」設計的；四狀態計量表（`state-*`）的幾何是為 20px 的行內位置收斂的橫向膠囊，放到 56px 會讀成刪除記號——實際試過並否決，見 `HomeNightNotice.vue`。

> **2026-09-04：32 檔位的「卡片標題圖示」有共用類別了。** 圖示與標題同列、垂直置中的那個排法叫 `.section-heading`，在 `apps/web/src/assets/app.css`。
>
> 它原本只寫在 `DataSettingsPage.vue` 的 scoped style 裡，所以**別的頁想用也用不到**——清點 24 個頁面：通知設定頁 5 張卡 0 個標題圖示、帳號與雲端資料頁 4 張卡 0 個，而 `ProductsPage` 只好自己另刻一個 `.gear-section-heading`。使用者的回報是「有些頁面沒有 icon 或裝飾，我覺得很空」，成因不是品味，是這條規則被鎖住了。（同一個坑的第三次：`.time-option`、`.time-picker--invalid` 都是這樣搬出來的。）
>
> **什麼時候用，判準是「這張卡要不要被掃讀」**：
>
...
> **⚠️ 讀這一節的方式**：下面的 `>` 引言區塊是**按日期累積的歷史紀錄**，記的是「當時改了什麼、為什麼」，**不是現況**。其中有些已被後續改動推翻——例如 2026-08-24 那條「`--surface-primary` 改回 canvas」，之後又改成了 `surface-soft`。
>
> **判斷現況只看兩個地方**：本節開頭那張落差表，以及 `packages/ui/src/styles.css`／`apps/web/src/assets/app.css` 的實際程式碼。歷史區塊用來追查「為什麼會變成這樣」，不要拿它當規範。
...
| **深色表面體系** | 範圍規則見第二節〈深色表面的使用範圍〉（2026-09-03 新增）：只用於即時核心看板與分享圖主角卡。2026-08-23 規劃的三個落點（設定流程步驟外框、產品標示編輯、五日預報強調欄）仍未實作；`surface-dark-elevated`／`surface-dark-soft`／`on-dark-soft` 為其配套 | 原規劃的三個落點都還是淺色。`--surface-inverse` 已有第一個消費者（分享卡，2026-09-01），但那不在原規劃裡；`--surface-dark-elevated`／`--surface-dark-soft` 仍是 0 次，`--color-surface-dark` 只出現在 `--surface-inverse` 的定義裡 | **規範有效，部分套用。** 2026-08-23 深色面板從「首頁倒數」改定位到這三處（見第七節），但改定位之後沒有實作。**這一列的用途是防止誤判**——2026-08-30 的未使用宣告稽核一度把這 4 個 token 判成「因設計變更而失效、可收掉」，差點刪掉還有效的規範。套用範圍是三個元件的視覺重做加深色底對比度重驗，要另外排程，不併進 token 收斂。詳見 `docs/decisions/2026-08-30-unused-declaration-audit.md` |
...
>
> 1. **`--surface-primary` 改回 `{colors.canvas}`**，取代 2026-08-22 一度指向的 `{colors.surface-card}`。本節第五節的 `app-card` 規範本來就寫「背景 canvas、1px hairline 邊框」，2026-08-22 那次色彩套用其實是改錯方向；Claude Design 的下游元件庫一直維持 canvas＋hairline 版本。這次改動會讓 `app-card`、輸入框、bottom sheet 等所有引用 `--surface-primary` 的元件變成跟頁面同色，只靠 hairline 分隔——`HomeCountdown`／`ReminderPanel`／`BottomNavigation` 三處直接引用 `{colors.surface-card}`，不受影響。
>
>    **⚠️ 2026-08-30 更正：這一條已經不是現況。** `--surface-primary` 目前指向 `{colors.surface-soft}` `#F7EDE1`（不是 canvas），第五節的 `app-card` 規範也已改寫成 `{colors.surface-soft}`。上面那句「規範本來就寫背景 canvas」與原本附的行號都已失效。**本節是歷史紀錄，判斷現況請看第五節與 `packages/ui/src/styles.css`。**
>
...
動效的定位：這個產品的情緒基調是**耐心**——核心機制是兩小時的等待，文案也刻意寫「這是協助你記得補擦的提醒，不是安全曝曬時間保證」。動效要支撐這個 tone，不是展示流暢度。2026-08-29 重訂為五條，2026-09-04 改寫第一條並補上第六條。

#### 一、動效只做兩件事

**讓轉場被看見**（`opacity`），以及**讓版面的改變是連續的**（高度塌陷、指示器展開）。除此之外不做表演。

2026-09-04 從「只用 `opacity`，唯一例外是內容進場」改寫成這條原則。原因是例外正在累積：內容進場的 4px 上移是第一個，收合時的高度塌陷是第二個，下排導覽指示器的橫向展開是第三個——再用白名單寫下去，「唯一例外」這句話就是假的，而**過期的規範比沒有規範更危險**。原則可以拿來判斷還沒發生的新案例，白名單不能。

界線仍然明確。以下一律不做：

- **彈跳、回彈、overshoot**（緩動曲線的控制點也不得超過 1）
- **旋轉**
- **漣漪（ripple）**——它的視覺語言是「擴散的能量」，跟這一節開頭寫的「耐心」直接相衝
- **裝飾性縮放**：hover 放大、按下縮小、圖示自己脹縮

狀態切換、圖示內部、hover、loader 仍然一律**純 `opacity`**。

**已核可的非 opacity 動效**（清單是封閉的，要加新的先裁決，不要就地擴張）：

| 動效                               | 用在               | 為什麼需要                                                      | 裁決       |
| ---------------------------------- | ------------------ | --------------------------------------------------------------- | ---------- |
| `{motion.motion-rise}`（4px 上移） | 內容進場           | 在 `{colors.canvas}` 這種低對比暖底上，純 0→1 淡入幾乎察覺不到  | 2026-08-29 |
| 高度塌陷                           | 展開區塊收合       | 純 opacity 淡出會讓下方內容瞬間往上跳；那個跳動比動畫本身更搶眼 | 2026-09-04 |
| 指示器橫向展開                     | 下排導覽的選取藥丸 | 直接淡入讀不出「移到這一項」；展開才有方向感                    | 2026-09-04 |
...
1. **Lucide 已移除（2026-08-29 結案）。** 8 個檔案的 `import ... from "@lucide/vue"` 全部換成 `<Icon name="..." />`，依賴也已從 `apps/web/package.json` 刪除。替換時順帶把散落的尺寸（25／26／22／18／17）收斂回系統的 16／20／24 三檔。剩下的唯一圖示待辦是衛教部位示意，但它的規格（要畫幾個、抽象到什麼程度）尚未裁決——數量請以第 6 點為準，不要用這裡以外的數字開工。
...
6. **衛教部位示意尚未設計，而且數量還沒定。** **動手前要先裁決畫幾個、抽象到什麼程度。**

   **2026-08-30 更正實際數量**：先前文件裡流傳三個互相衝突的數字——本節舊版與 `docs/design/icon-system/README.md` 的第十節寫「10 個」、同份 README 的造型語言表寫「16 個」、更早的規格 4.6 寫「七部位」。實際核對 `packages/contracts` 的 `BodyZoneCodeSchema` 與 `apps/web/src/features/setup/setupCatalog.ts`：

   **15 個具名部位**——額頭、鼻部與雙頰、臉部下半部、耳朵、嘴唇、頭皮、前頸、後頸、肩膀、軀幹前、軀幹後、手臂、手背、腿部、腳背——**另有 `custom` 供自訂**，所以契約列舉共 16 個值。

   舊的「10 個」是把臉部三處、頸部兩處、軀幹前後各自合併後的計數，不是程式碼裡的實際項目；「16 個」則是把 `custom` 算進去。**不要用 7 或 10 開工。**
...
9. ~~`/reminder/reapply` 的最終顯示形式未定~~ **已實作（2026-08-30 核對）。** `ReapplyPage.vue` 273 行、路由 `reminder-reapply` 都在，部位選擇、建議部位、產品指派與確認流程都已接好。這一列原本寫「Sitemap 文件標註仍需在 wireframe 階段確認」，那個階段早已過去。
