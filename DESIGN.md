...
> **字標是例外（2026-08-22）**：Logo 的「防曬晴報員」五個字用**思源圓體**（Source Han Round）Medium／500，不是襯線體。理由是字標緊鄰圖標，而圖標的造型語言是「實心圓點＋膠囊線條、端點全圓、**不使用尖角**」（第八節），明體的尖角收筆與它直接衝突。圓體保留黑體結構、只磨圓端點，呼應膠囊語言又不越過「不可愛化」那條線。
>
> 這個例外**不擴及標題與內文**——那兩者維持上表規則。Logo 有自己的字體是常見且正當的做法。細節與待辦見 [`docs/design/logo-concepts/README.md`](docs/design/logo-concepts/README.md)。
