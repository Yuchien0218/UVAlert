...
## 2026-08-08 更新：sitemap 缺口全部補完

本文件原稿寫於 2026-08-07，當時 S-09、S-10、產品頁改版與本機資料管理都還沒實作。之後這幾項陸續完成，各節標題的狀態標記已同步更新，但**各節內文仍是 08-07 當下的描述**，讀到「待實施」字樣時以標題的標記為準。

| 項目 | 08-07 | 現況 |
| --- | --- | --- |
| S-09 回報狀況 `/reminder/report` | ❌ 路徑不存在 | ✅ 六種事件（四種一般原因＋入水／離水） |
| S-10 最近事件更正 `/reminder/event/:id/correct` | ⚠️ 表單未做 | ✅ replace 與 void，涵蓋回報狀況與補擦紀錄 |
| 產品頁改版 `/products` | ⏳ 無 gearCategory | ✅ 四個品類的裝備清單 |
| 新增／編輯裝備 | ❌ 不存在 | ✅ `/products/new`、`/products/:id/edit` |
| 本機資料管理 `/settings/data` | ⚠️ placeholder | ✅ 摘要、匯出與三段清除 |
| ActionKind 落點 | 13 個裡 11 個 | ✅ **13 個全數有真實落點** |

**尚未涵蓋**：S-09 規格第一層十項事件裡的四項——進入室內／返回戶外（`context_changed`）、更新衣物狀態、使用產品後感到不適。這三類走各自的命令路徑，是另一條全端切片。
...
**實現狀態**（2026-08-08 完成）
- ✅ 兩層表單：第一層事件選擇、第二層確認部位／水上 interval／時間
- ✅ 涵蓋六種事件：四種一般原因＋入水／離水
- ✅ 預選規則依規格：洗手預選手背、下水預選外露部位、離水沿用起點集合且鎖定
- ✅ `resolve_water_start` 一併接上（第二層即處理入水時間未知）
- ⏳ 規格第一層另有四項走其他命令路徑：進入室內／返回戶外、更新衣物狀態、產品不適

---

### ✅ S-10：最近事件更正 `/reminder/event/:id/correct`
...
**實現狀態**（2026-08-08 完成）
- ✅ 更正表單已實作，replace 與 void 皆支援
- ✅ 涵蓋回報狀況事件與補擦紀錄（後者以 confirmation group 為單位）
- ✅ `CorrectionSuccessors` 表開始使用，擋跨 transaction 的並發分支
- ✅ 路徑已存在（`requiresActiveSession`）
- ✅ 最近事件清單已接上提醒頁，每列可點並正確帶入事件 id
- ✅ 事件流讀取路徑完成：`getCurrentSessionEventStream` +
  `createSessionEventsController`（10 則測試）
- ✅ 清單已濾掉被更正掉的舊版，target 必為唯一有效 leaf
...
**實現狀態**（2026-08-08 完成）
- ✅ gearCategory 四品類，只有 sunscreen 進 reducer
- ✅ 新增 purchaseMonth／expiryDate／note／archivedAt，schemaVersion 1.1.0
- ✅ 舊資料 migration 與到期狀態同步
- ✅ 清單分「目前使用／過去紀錄」，每列明示該品類對提醒的影響
...
**實現狀態**（2026-08-08 完成）
- ✅ 選非 sunscreen 時收起所有只對 sunscreen 有意義的標示欄位並明示不影響倒數
...
**實現狀態**（2026-08-08 完成）
- ✅ 編輯、移至過去紀錄、恢復、刪除
- ✅ 既有 sunscreen 不得改為純紀錄品類；回報過異常／不適者不提供直接恢復
...
**實現狀態**
- ✅ 頁面與路由已實作
- ⚠️ 內容全未通過必要審查，審查閘門正確擋下，總覽顯示「目前沒有可查看的內容」

---

### ⚠️ 海邊防曬 Q&A `/help/beach`
...
### ⚠️ 運作說明 `/help/how-it-works`

**規格**
- 也是 `view_conservative_reminder` 的目的地（時鐘不可信且離線）

**實現狀態**
- ✅ 頁面已實作並接上該 ActionKind
- ⚠️ 內容未過審，顯示「內容正在審查」

---

### ⚠️ 特殊狀況 `/special-situation`

**規格**
- 處理產品異常、不適等情況

**實現狀態**
- ✅ 頁面已實作，「查看處理說明」次要 CTA 導向此頁
- ⚠️ 分類與說明未過審，顯示「此功能尚未開放」
...
**實現狀態**
- ✅ 已從 `/more` 移到獨立路由 `/settings/display`
- ⏳ 首次使用預設模式與規格不符（實作回傳 system，規格為日間高對比），留待配色重做時處理
...
**實現狀態**（2026-08-08 完成）
- ✅ 摘要六項、匯出、三段清除同置一頁
- ✅ 匯出排除裝置識別碼、ownerKey 與事件的 commandId／idempotencyKey
- ✅ 清除產品與歷史時保留進行中的提醒；清除全部逐條列出範圍
...
| S-09 回報狀況 | ✅ | P0 | ✓ 完成 | 六種事件；另四項事件屬其他命令路徑 |
| S-10 事件更正 | ✅ | P0 | ✓ 完成 | replace／void，含補擦紀錄 |
| 產品頁改版 | ✅ | P0 | ✓ 完成 | 四品類裝備清單 |
...
| 本機匯出 | ✅ | P0 | ✓ 完成 | 版本化 JSON，不上傳 |
| 本機資料管理 | ✅ | P0 | ✓ 完成 | 摘要、匯出與三段清除 |
...
- **實現狀態**：2026-08-08 完成，S-11／S-12／S-13 三頁一併補齊
- **對其他流程的影響**：影響 S-11、S-12、S-13
- **下游依賴**：數據模型擴充、Copy Deck 審查

### 4. /help 幫助中心 ⚠️
- **分支影響**：從多個入口進入幫助
- **實現狀態**：機制完成，內容全未過審，閘門正確擋下
