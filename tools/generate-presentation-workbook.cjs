...
const slides = [
  ...
  {
    id: "P.9",
    title: "本機是主體，後端提供網路服務",
    time: "35 秒",
    purpose: "用非工程背景也能理解的方式說明系統分工。",
    layout: "左右分區：裝置端 IndexedDB／Supabase 網路服務，中間標示選配同步。",
    text: [
      "本機 IndexedDB｜提醒、事件與設定",
      "Supabase｜UV 代理與快取、選配同步、推播、回饋與帳號刪除",
      "核心本機提醒不需登入；即時 UV 與網路服務需要連線",
    ],
    assets: ["兩欄架構圖", "使用者主動登入的虛線箭頭"],
    notes: "架構原則是本機為主體，後端是選配與網路服務層。提醒、事件與設定先存在瀏覽器 IndexedDB；Supabase 負責 UV 資料代理與快取、登入後的雲端同步、背景推播、匿名回饋與帳號刪除。不登入仍可使用本機核心流程，但 UV 更新、雲端同步與遠端推播需要網路。",
  },
  {
    id: "P.10",
    title: "從設計判斷到可運作產品",
